...
def inertia_tensor_point_cloud(points, com = None):
    masses = points.shape[0]
    
    # Compute center of mass
    if com is None:
        com = np.sum(points, axis=0)/masses
    r = points - com  # relative coordinates
    x, y, z = r[:,0], r[:,1], r[:,2]
...
def gyration_tensor_point_cloud(points, com = None):
    masses = points.shape[0]
    # Compute center of mass
    if com is None:
        com = np.sum(points, axis=0)/masses
    normed_points = points - com[None,:]
    S = np.einsum('im,in->mn', normed_points,normed_points)/len(points)
    return S
...
def inertia_tensor_field(cm, OutArray):
    """
...
def gyration_tensor_field(cm, OutArray):
    """
...
def droplet_deformation(values1, m):
    """
